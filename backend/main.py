...
async def _load_message_history(session_id: str) -> list[dict]:
    ...
    If the session has a ``session_summary`` (generated by the rolling-window
    summariser), it is prepended as a synthetic user/assistant exchange so
    Claude has the earlier story context without consuming the full token
    budget.
    ...
    # Prepend the condensed earlier-session context if available
    if summary:
        history.append({
            "role": "user",
